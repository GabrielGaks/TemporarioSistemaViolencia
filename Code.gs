...
function doPost(e) {
  try {
    ...
    // Tenta pegar dados de diferentes formas (otimizado)
...
function saveRegistro(formData) {
  try {
    ...
    function extrairSiglaEscola(nomeCompleto) {
      ...
      // Verifica se já é uma sigla (curta, apenas letras maiúsculas, sem espaços)
      // Siglas geralmente têm 2-6 caracteres e são apenas letras maiúsculas
      const valorLimpo = nomeCompleto.trim();
      const isSigla = /^[A-Z]{2,6}$/.test(valorLimpo) && valorLimpo.length <= 6;
      
      // Se já é uma sigla válida, preserva como está
      if (isSigla) {
        return valorLimpo;
      }
      
      // Caso contrário, extrai a sigla do nome completo
    ...
    // Adiciona a linha na planilha - OTIMIZADO: uma única operação
...
function listarRegistros() {
  try {
    ...
    // Lê todos os dados (pula linha 1 do cabeçalho) - OTIMIZADO: uma única leitura
    const range = sheet.getRange(2, 1, lastRow - 1, TOTAL_COLS);
    const valores = range.getValues();
    
    const registros = [];
    
    // OTIMIZADO: Processa dados em batch sem logs desnecessários
...
function atualizarRegistro(dados) {
  try {
    ...
    // Funções auxiliares otimizadas (sem logs)
    function converterSimNao(valor) {
      if (!valor) return '';
      const valorStr = String(valor).trim();
      if (valorStr === '' || valorStr === 'Não informado') return '';
      if (valorStr === 'Sim') return 'S';
      if (valorStr === 'Não') return 'N';
      return valorStr;
    }
    ...
    function extrairSiglaEscola(nomeCompleto) {
      ...
      // Verifica se já é uma sigla válida (2-6 letras maiúsculas, sem espaços)
      const isSiglaValida = /^[A-Z]{2,6}$/.test(valorLimpo) && valorLimpo.length <= 6;
      ...
      // Verifica se é uma sigla que precisa ser normalizada (tem espaços, minúsculas, etc)
      // Remove espaços e converte para maiúsculas
      const valorNormalizado = valorLimpo.replace(/\s+/g, '').toUpperCase();
      const isSiglaNormalizada = /^[A-Z]{2,6}$/.test(valorNormalizado) && valorNormalizado.length <= 6;
      
      // Se após normalização é uma sigla válida, retorna normalizada
      if (isSiglaNormalizada) {
        return valorNormalizado;
      }
      
      // Verifica se o valor é muito curto para ser um nome completo (provavelmente é uma sigla)
      // Se tem menos de 10 caracteres e não contém espaços significativos, pode ser sigla
      if (valorLimpo.length <= 8 && !valorLimpo.includes(' ')) {
        // Se parece com sigla (apenas letras), preserva em maiúsculas
        if (/^[A-Za-z]+$/.test(valorLimpo)) {
          return valorLimpo.toUpperCase();
        }
      }
      
      // Se o valor começa com CMEI ou EMEF, mas é muito curto após remover o prefixo,
      // provavelmente é uma sigla que foi salva incorretamente
      const semPrefixo = valorLimpo.replace(/^(CMEI|EMEF)\s+(TI\s+)?/i, '').trim();
      if (semPrefixo.length <= 6 && semPrefixo.length >= 2 && !semPrefixo.includes(' ')) {
        // Se parece com sigla após remover prefixo, preserva
        if (/^[A-Za-z]+$/.test(semPrefixo)) {
          return semPrefixo.toUpperCase();
        }
      }
      
      // Caso contrário, extrai a sigla do nome completo
      let nome = valorLimpo.replace(/^(CMEI|EMEF)\s+(TI\s+)?/i, '');
      const ignorar = ['de', 'da', 'do', 'das', 'dos', 'e', 'a', 'o', 'as', 'os'];
      const palavras = nome.split(' ').filter(p => p.length > 0);
      const sigla = palavras
        .filter(palavra => !ignorar.includes(palavra.toLowerCase()))
    ...
    // Atualiza a linha - OTIMIZADO: uma única operação de escrita
